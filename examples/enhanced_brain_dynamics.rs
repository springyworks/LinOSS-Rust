//! Enhanced Brain Dynamics with Energy Conservation
//! 
//! This version addresses the convergence issues by:
//! - Using smaller time steps for better numerical stability
//! - Adding energy injection to counter numerical damping
//! - Using Runge-Kutta integration for higher accuracy
//! - Monitoring and maintaining chaotic behavior
...
// Enhanced constants for stable chaos
const MAX_TRAJECTORY_POINTS: usize = 2000;
const SIMULATION_SPEED: u64 = 30; // Faster updates
const DT: f64 = 0.005; // Smaller time step for stability
const CANVAS_X_BOUNDS: [f64; 2] = [-50.0, 50.0];
const CANVAS_Y_BOUNDS: [f64; 2] = [-50.0, 50.0];
const ENERGY_INJECTION_FACTOR: f64 = 0.001; // Counteract numerical damping
...
struct BrainRegion {
    ...
    initial_energy: f64, // Track initial energy for conservation
    last_velocity: (f64, f64, f64), // For better integration
}

impl BrainRegion {
    fn new(name: String, initial_pos: (f64, f64, f64), params: LorenzParams, color: Color) -> Self {
        let initial_energy = initial_pos.0 * initial_pos.0 + initial_pos.1 * initial_pos.1 + initial_pos.2 * initial_pos.2;
    ...
    fn init_dlinoss(&mut self, device: &<B as Backend>::Device) {
        let layer_config = DLinossLayerConfig::new_dlinoss(3, 16, 3); // Larger hidden layer
    ...
    fn lorenz_step_rk4(&mut self, dt: f64, coupling_input: (f64, f64, f64)) {
        ...
        let dz = (k1z + 2.0*k2z + 2.0*k3z + k4z) / 6.0;

        self.position.0 += dx * dt;
        self.position.1 += dy * dt;
        self.position.2 += dz * dt;
        
        self.last_velocity = (dx, dy, dz);
    }

    // Enhanced dLinOSS with controlled influence
    fn dlinoss_step(&mut self) {
        if let (Some(model), Some(input)) = (&self.dlinoss_model, &self.dlinoss_input) {
            ...
            // Reduced influence to maintain chaos
            let influence_factor = 0.005;
    ...
    // Energy injection to counteract numerical damping
    fn inject_energy(&mut self) {
        let current_energy = self.position.0 * self.position.0 + self.position.1 * self.position.1 + self.position.2 * self.position.2;
        let energy_ratio = self.initial_energy / current_energy;
        
        // If energy is significantly reduced, inject a small amount
        if energy_ratio > 1.1 {
            let injection = ENERGY_INJECTION_FACTOR * (energy_ratio - 1.0);
            let vel_magnitude = (self.last_velocity.0 * self.last_velocity.0 + 
                               self.last_velocity.1 * self.last_velocity.1 + 
                               self.last_velocity.2 * self.last_velocity.2).sqrt();
            
            if vel_magnitude > 0.0 {
                self.position.0 += injection * self.last_velocity.0 / vel_magnitude;
                self.position.1 += injection * self.last_velocity.1 / vel_magnitude;
                self.position.2 += injection * self.last_velocity.2 / vel_magnitude;
            }
        }
    }
    ...
    fn get_velocity_magnitude(&self) -> f64 {
        (self.last_velocity.0 * self.last_velocity.0 + 
         self.last_velocity.1 * self.last_velocity.1 + 
         self.last_velocity.2 * self.last_velocity.2).sqrt()
    }
}
...
impl EnhancedBrainDynamicsApp {
    fn new() -> Self {
        let device = chosen_backend::get_device();
        
        // Create regions with slightly different initial conditions for sustained chaos
        let mut regions = vec![
            BrainRegion::new(
        ...
        Self {
            regions,
            paused: false,
            status_message: "Enhanced Brain Dynamics - Press 'p' pause, 'l' dLinOSS, 'e' energy injection, 'q' quit".to_string(),
            simulation_time: 0.0,
    ...
    fn on_tick(&mut self) {
        ...
        for (i, region) in self.regions.iter_mut().enumerate() {
            // Use improved RK4 integration
            region.lorenz_step_rk4(DT, coupling_matrix[i]);
            
            // Apply dLinOSS if enabled
            if self.use_linoss {
                region.dlinoss_step();
            }
            
            // Inject energy to maintain chaos if enabled
            if self.energy_injection_enabled {
                region.inject_energy();
            }
    ...
    fn toggle_pause(&mut self) {
        ...
        self.status_message = if self.paused {
            ...
            "Enhanced Brain Dynamics - Press 'p' pause, 'l' dLinOSS, 'e' energy injection, 'q' quit".to_string()
        };
    }
    ...
    fn toggle_linoss(&mut self) {
        ...
        self.status_message = format!(
            "dLinOSS: {} - Press 'p' pause, 'l' toggle dLinOSS, 'e' energy injection, 'q' quit",
            if self.use_linoss { "ON" } else { "OFF" }
        );
    }

    fn toggle_energy_injection(&mut self) {
        self.energy_injection_enabled = !self.energy_injection_enabled;
        self.status_message = format!(
            "Energy Injection: {} - Maintains chaotic behavior",
            if self.energy_injection_enabled { "ON" } else { "OFF" }
        );
    }
}

// UI drawing function (similar to original but with enhanced status)
fn draw_ui(f: &mut Frame, app: &EnhancedBrainDynamicsApp) {
    let main_chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Percentage(65), // Chart area
    ...
    let chart = Chart::new(datasets)
        .block(
            TuiBlock::default()
                .title("Enhanced Brain Dynamics: Sustained Chaos (Carhart-Harris Model)")
                .borders(Borders::ALL),
        )
        .x_axis(
    ...
    let status_block = TuiBlock::default().borders(Borders::ALL).title("Enhanced System Status");
    ...
    let status_chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Length(1), // Energy injection status
            Constraint::Length(1), // Velocity monitoring
        ].as_ref())
        .split(status_inner);
    ...
    let time_paragraph = Paragraph::new(format!("Simulation Time: {:.2}s | dt: {:.3}", app.simulation_time, DT));
    f.render_widget(time_paragraph, status_chunks[1]);

    let linoss_paragraph = Paragraph::new(format!(
        "dLinOSS Neural Dynamics: {} | Coupling: {:.3}", 
        if app.use_linoss { "ACTIVE" } else { "DISABLED" },
        app.coupling_strength
    ));
    f.render_widget(linoss_paragraph, status_chunks[2]);

    let energy_paragraph = Paragraph::new(format!(
        "Energy Injection: {} | RK4 Integration: ACTIVE",
        if app.energy_injection_enabled { "ENABLED" } else { "DISABLED" }
    ));
    f.render_widget(energy_paragraph, status_chunks[3]);

    // Show velocity magnitudes to monitor chaos
    let velocities: Vec<f64> = app.regions.iter().map(|r| r.get_velocity_magnitude()).collect();
    let velocity_paragraph = Paragraph::new(format!(
        "Velocity: PFC:{:.3} DMN:{:.3} THAL:{:.3}",
        velocities.first().unwrap_or(&0.0),
        velocities.get(1).unwrap_or(&0.0),
        velocities.get(2).unwrap_or(&0.0)
    ));
    f.render_widget(velocity_paragraph, status_chunks[4]);

    // Brain regions info
    let info_block = TuiBlock::default().borders(Borders::ALL).title("Brain Regions (Enhanced)");
...
fn run_app(terminal: &mut Terminal<CrosstermBackend<Stdout>>) -> Result<(), Box<dyn Error>> {
    let mut app = EnhancedBrainDynamicsApp::new();
        ...
        if crossterm::event::poll(timeout)? {
            if let event::Event::Key(key) = event::read()? {
                match key.code {
                    ...
                    KeyCode::Char('e') => app.toggle_energy_injection(),
                    KeyCode::Char('+') => {
