...
impl<B: Backend> DLinossLayer<B> {
    ...
    pub fn forward(&self, input: Tensor<B, 3>) -> Tensor<B, 3> {
        ...
        for t in 0..seq_len {
            ...
            // Project input
            let projected_input = input_t.matmul(self.b_matrix.clone().transpose());
            ...
            hidden_states.push(hidden_state.clone().unsqueeze_dim(1));
        }
        
        // Stack all hidden states
        let all_hidden_states = Tensor::cat(hidden_states, 1);
        
        // Output projection: y_t = h_t * C^T + x_t * D^T
        let output_projection = all_hidden_states.reshape([batch_size * seq_len, d_model])
            .matmul(self.c_matrix.clone().transpose())
            .reshape([batch_size, seq_len, self.c_matrix.dims()[0]]);
            
        let direct_projection = input.reshape([batch_size * seq_len, input.dims()[2]])
            .matmul(self.d_matrix.clone().transpose())
            .reshape([batch_size, seq_len, self.d_matrix.dims()[0]]);
    ...
    fn apply_learnable_damping(&self, state: Tensor<B, 3>) -> Tensor<B, 3> {
        if let (Some(damping_coeffs), Some(_damping_scales)) = (&self.damping_coeffs, &self.damping_scales) {
            let [batch_size, seq_len, d_model] = state.dims();
            let num_oscillators = d_model / 2;
            
            // Simplified damping: apply learned damping factor to velocity components
            let mut damped_state = state.clone();
            
            for osc in 0..num_oscillators {
                let vel_idx = osc * 2 + 1; // velocity component index
                
                // Get average damping for this oscillator (simplified)
                let osc_damping = damping_coeffs.clone().slice([osc..osc+1, 0..1]).squeeze(1).squeeze(0);
                let damping_factor = (-osc_damping * self.delta_t).exp();
                
                // Apply damping to velocity component
                let damped_vel = damped_state.clone()
                    .slice([0..batch_size, 0..seq_len, vel_idx..vel_idx+1]) * damping_factor;
