// ⚡ CLI Runner for Bidirectional GPU Communication Test ⚡
// Executes the comprehensive GPU-only data flow test with 3D WGPU integration
...
async fn run_bidirectional_gpu_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🎮 Features: 3D WGPU compute pipeline integration");
    println!();
    ...
    for &osc_count in &oscillator_counts {
        ...
        for step in 1..=test_steps {
            // Step 1: GPU computation (both Burn tensors + WGPU compute)
    ...
    println!("✅ 3D WGPU compute pipeline operational");
