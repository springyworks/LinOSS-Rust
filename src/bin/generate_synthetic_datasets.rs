    ...
    for i in 0..n_samples {
        ...
        for j in 0..n_features {
            ...
            let pattern = ((i as f32 * 0.1 + j as f32 * 0.05).sin()
                + (i as f32 * 0.07 + class as f32).cos())
                * 0.2;
...
fn save_dataset_info(
    ...
    writeln!(
        file,
        "- `{}_features.npy` - Features in NumPy array format",
        dataset_name
    )?;
    writeln!(
        file,
        "- `{}_labels.npy` - Labels in NumPy array format",
        dataset_name
    )?;
    ...
    writeln!(
        file,
        "let features: Array2<f32> = read_npy(\"{}_features.npy\")?;",
        dataset_name
    )?;
    writeln!(
        file,
        "let labels: Array1<u8> = read_npy(\"{}_labels.npy\")?;",
        dataset_name
    )?;
    writeln!(file, "```")?;
    writeln!(file)?;
