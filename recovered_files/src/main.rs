...
use nalgebra::DVector; // Use DVector instead of generic Vector
use linoss_rust::linoss::{
    model::FullLinossModel, model::FullLinossModelConfig, block::LinossBlockConfig, layer::LinossLayerConfig,
};
use linoss_rust::{LinossError}; // TensorData for creation, Shape for dimensions // Example backend
...
fn main() -> Result<(), LinossError> {
    ...
    input_sequence_nalgebra_vec.push(DVector::from_vec(vec![0.5f64]));
    // Time step 1: input value 1.0
    input_sequence_nalgebra_vec.push(DVector::from_vec(vec![1.0f64]));
    ...
    let mut output_sequence_nalgebra_vec: Vec<DVector<f64>> = Vec::new();
    ...
    for i in 0..sequence_len_n {
        ...
        if end_index <= output_data_f32.len() {
            ...
            output_sequence_nalgebra_vec.push(DVector::from_vec(nalgebra_vector_data));
        }
    }
